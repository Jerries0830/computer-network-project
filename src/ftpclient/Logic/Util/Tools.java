...
public class Tools {
    ...
    public static String getAddress(int port) {
        String[] host = getIPArray();
        int high = port / 256;
        int low = port - high * 256;
        return String.format("%s,%s,%s,%s,%s,%s", host[0], host[1], host[2], host[3], high, low);
    }

    public static int getReturnCode(String response) {
    ...
    //递归获取文件夹内所有文件
    public static void getFilesInDir(File directory, ArrayList<String> files) {
        for (File file : Objects.requireNonNull(directory.listFiles())) {
            if (file.isDirectory()) {
                getFilesInDir(file, files);
